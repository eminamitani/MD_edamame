...
int main() {
    ...
    //定数
    const RealType dt = 5e-3;
    const RealType cutoff = 2.0;
    const RealType margin = 1.0;

    const RealType tau = dt * 100;
    const IntType chain_length = 4;
    const RealType temperature = 1.0;

    //現在時刻を記録
    auto start = std::chrono::steady_clock::now();

    //MDオブジェクトの実体化
    MD md = MD(dt, cutoff, margin, atoms, device);

    //速度の初期化
    md.init_vel_MB(1.0);

    //シミュレーションの開始
    md.NVT_LJ(2e+3, chain_length, tau, temperature);
