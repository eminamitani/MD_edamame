...
//---------------------------------------------------------------
torch::Tensor conf_tensor = torch::zeros({N, deg}, torch::kFloat64);  
torch::Tensor velo_tensor = torch::zeros({N, deg}, torch::kFloat64);  
torch::Tensor force_tensor = torch::zeros({N, deg}, torch::kFloat64);

auto conf = conf_tensor.accessor<double, 2>();
auto velo = velo_tensor.accessor<double, 2>();
auto force = force_tensor.accessor<double, 2>();
...
void generate_NL() {
    ...
    std::memcpy(NL_config, conf_tensor.data_ptr<double>(), N * deg * sizeof(double));
}
//---------------------------------------------------------------
void calc_force() {
...
void NVE(const double tsim, bool is_save) {
    ...
    while (t < steps) {
        thermostats.update(velo_tensor, 0.5 * torch::sum(torch::pow(velo_tensor, 2)), dt_tensor);
        velocity_update();
        position_update();
