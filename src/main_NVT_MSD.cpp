...
int main(){
    ...
    const RealType T_targ = 2000.0;          //目標温度 (K)
    ...
    //保存先
    const std::string traj_path = "./NS2_T2000.xyz";
    ...
    //シミュレーションの実行（緩和）
    md.NVT(t_eq, bussi_thermostat, "log", false, "./output_NS2_T2000_eq.xyz");
    std::cout << "緩和完了" << std::endl;
    md.reset_step();

    //シミュレーションの実行（production run）
    md.NVT(t_eq, bussi_thermostat, "log", true, "./output_NS2_T2000_production.xyz");
